...
export default function Content() {
  return (
    <>
      <p>
        一周前，Perry 还是一个带有 UI 工具包的编译器。你可以编写 TypeScript，将其编译
        为原生二进制文件，并在六个平台上发布。这就是全部。今天故事更大了：Perry 正在成为一个生态系统。三个数据库 ORM、通用推送通知、
        带有 App Store 和 Play Store 发布的分布式构建、React 兼容层，
        以及自动化应用验证 —— 全部在上周发布。
      </p>
      <p>
      ...
      <h2>三个数据库 ORM：Prisma API，原生性能</h2>
      <p>
      ...
      <h2>Perry Hub + Builder：分布式云构建</h2>
      <p>
